...
$('#regione').on('change', function (e) {
    ...
    if ($('#regione').val() !== '-') {
        startBlockUILoad("#provincia_div");
        $("#provincia").append('<option value="-">Seleziona Provincia</option>');
        $.get(contextPath + '/Login?type=getProvincia&regione=' + $('#regione').val(), function (resp) {
            var json = JSON.parse(resp);
            for (var i = 0; i < json.length; i++) {
                $("#provincia").text('<option value="' + json[i].value + '">' + json[i].desc + '</option>');
            }
...
$('#provincia').on('change', function (e) {
    ...
    if ($('#provincia').val() !== '-') {
        startBlockUILoad("#comune_div");
        $("#comune").append('<option value="-">Seleziona Comune</option>');
        $.get(contextPath + '/Login?type=getComune&provincia=' + $('#provincia').val(), function (resp) {
            var json = JSON.parse(resp);
            for (var i = 0; i < json.length; i++) {
                $("#comune").text('<option value="' + json[i].value + '">' + json[i].desc + '</option>');
            }
...
var KTDatatablesDataSourceAjaxServer = function () {
    var initTable1 = function () {
        ...
        table.DataTable({
            ...
            processing: true,
            pageLength: 10,
            ajax: contextPath + '/QueryMicro?type=searchSedi&referente=' + $('#referente').val()
                    + "&regione=" + $('#regione').val()
...
function refresh() {
    $("#toolbar").css("display", "none");
    $('html, body').animate({scrollTop: $('#offsetresult').offset().top}, 500);
    load_table($('#kt_table_1'), contextPath + '/QueryMicro?type=searchSedi&referente=' + $('#referente').val()
            + "&regione=" + $('#regione').val()
            + "&provincia=" + $('#provincia').val()
            + "&comune=" + $('#comune').val());
}
...
function changestatus(id, status) {
    ...
    $.ajax({
        type: "POST",
        url: contextPath + '/OperazioniMicro?type=validateAula&id=' + id + "&status=" + statusdest,
        success: function (data) {
            closeSwal();
            var json = JSON.parse(data);
            if (json.result) {
